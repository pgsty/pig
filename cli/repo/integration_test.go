...
// Test that needsEL10VersionFix correctly identifies pgdg repos
func TestNeedsEL10VersionFix(t *testing.T) {
	// Save original values
	origMajor := config.OSMajor
	origArch := config.OSArch
	origType := config.OSType

	// Set up EL10 aarch64 environment
	config.OSMajor = 10
	config.OSArch = "aarch64"
	...
	tests := []struct {
		...
		{"pgdg11", true},
		{"pgdg10", true},
		{"pgdg9", false}, // Too short
		{"pgdg", false},  // No version number
		{"PGDG17", true}, // Case insensitive
		{"pigsty", false},
		{"epel", false},
		{"pgdg-common", false}, // Has dash, not a version number
	}

	for _, tt := range tests {
		t.Run(tt.repoName, func(t *testing.T) {
			repo := &Repository{Name: tt.repoName}
			result := repo.needsEL10VersionFix()
			if result != tt.expected {
				t.Errorf("needsEL10VersionFix() for repo '%s' = %v, expected %v", tt.repoName, result, tt.expected)
			}
		})
	}

	// Test with different OS configurations
	config.OSMajor = 9 // Not EL10
	repo := &Repository{Name: "pgdg17"}
	if repo.needsEL10VersionFix() {
		t.Error("Should not apply fix for EL9")
	}

	config.OSMajor = 10
	config.OSArch = "x86_64" // Not aarch64
	if repo.needsEL10VersionFix() {
		t.Error("Should not apply fix for x86_64")
	}
