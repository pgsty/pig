...
func betaPGMajorString() string {
	return fmt.Sprintf("%d", ext.PostgresBetaMajorVersion)
}

func betaPGMode() string {
	return fmt.Sprintf("pg%d", ext.PostgresBetaMajorVersion)
}

func betaPGMainPackage() string {
	return fmt.Sprintf("pg%d-main", ext.PostgresBetaMajorVersion)
}

func betaPGVersionLine() string {
	return fmt.Sprintf("pg_version: %d", ext.PostgresBetaMajorVersion)
}
...
func TestMutateTemplateBetaPGEnablesBetaRepoModule(t *testing.T) {
	content := `all:
  vars:
    node_repo_modules: 'node,infra,pgsql'
    pg_version: 18
...
`
	got, _, warnings, err := mutateTemplate(content, mutationOptions{
		Mode:      "meta",
		PGVersion: ext.PostgresBetaMajorVersion,
	})
	if err != nil {
		t.Fatalf("mutateTemplate error: %v", err)
	}
	if !strings.Contains(got, "node_repo_modules: 'node,infra,pgsql,beta'") {
		t.Fatalf("expected beta repo module for PG%d, got:\n%s", ext.PostgresBetaMajorVersion, got)
	}
	if !strings.Contains(got, betaPGVersionLine()) || !strings.Contains(got, betaPGMainPackage()) {
		t.Fatalf("expected beta pg version and package alias replacement, got:\n%s", got)
	}
	if len(warnings) != 0 {
...
func TestMutateTemplateBetaPGRepoModuleIdempotent(t *testing.T) {
	// node_repo_modules already has beta (like dedicated beta templates), repo_modules does not:
  ...
  vars:
    ...
    pg_version: %d
    pg_packages: [ %s ]
`, ext.PostgresBetaMajorVersion, betaPGMainPackage())
	got, _, warnings, err := mutateTemplate(content, mutationOptions{
		Mode:      betaPGMode(),
		PGVersion: ext.PostgresBetaMajorVersion,
	})
...
func TestMutateTemplateBetaPGRepoModuleOrderVariants(t *testing.T) {
...
func TestMutateTemplateBetaPGRepoModuleMissingWarns(t *testing.T) {
...
func TestMutateTemplateBetaPGRepoModuleIgnoresComments(t *testing.T) {
...
func TestMutateTemplateBetaPGSkipsPinnedKernelModes(t *testing.T) {
	content := `all:
  vars:
    node_repo_modules: node,infra,pgsql
    pg_version: 17
...
func TestMutateTemplatePGMajorTemplateSkipsVersionRewrite(t *testing.T) {
	for _, tt := range []struct {
		...
		{mode: betaPGMode(), locked: ext.PostgresBetaMajorVersion},
		{mode: "pg20", locked: 20},
		{mode: "pg21", locked: 21},
	} {
...
func TestMutateTemplateBetaSkippedOnceMajorIsStable(t *testing.T) {
	// GA tripwire: once the current beta enters the stable active window, it
	// must no longer inject the beta repo module.
	oldActive := ext.PostgresActiveMajorVersions
	ext.PostgresActiveMajorVersions = []int{ext.PostgresBetaMajorVersion, 18, 17, 16, 15}
...
`
	...
	if !strings.Contains(got, betaPGVersionLine()) {
		t.Fatalf("expected pg_version updated to beta PG%d, got:\n%s", ext.PostgresBetaMajorVersion, got)
	}
	if len(warnings) != 0 {
		t.Fatalf("unexpected warnings: %v", warnings)
	}
...
func TestConfigureNativeBetaPGSuggestsDedicatedTemplate(t *testing.T) {
	tmp := t.TempDir()
	confDir := filepath.Join(tmp, "conf")
	if err := os.MkdirAll(confDir, 0755); err != nil {
		t.Fatalf("mkdir conf: %v", err)
...
`
	...
	if err := os.WriteFile(filepath.Join(confDir, betaPGMode()+".yml"), []byte("all: {}\n"), 0644); err != nil {
		t.Fatalf("write beta pg template: %v", err)
	}
	...
	for _, w := range data.Warnings {
		if strings.Contains(w, "pig sty conf -c "+betaPGMode()) {
			suggested = true
		}
	}
	if !suggested {
		t.Fatalf("expected beta pg template suggestion warning, got: %v", data.Warnings)
	}
...
func TestConfigureNativePGMajorTemplateSkipsVersionRewrite(t *testing.T) {
  ...
  vars:
    ...
    pg_packages: [ %s ]
`, ext.PostgresBetaMajorVersion, betaPGMainPackage())
	if err := os.WriteFile(filepath.Join(confDir, betaPGMode()+".yml"), []byte(template), 0644); err != nil {
		t.Fatalf("write template: %v", err)
	}
	...
	got := string(out)
	if !strings.Contains(got, betaPGVersionLine()) || strings.Contains(got, "pg_version: 16") {
		t.Fatalf("expected beta pg template to keep its pg_version, got:\n%s", got)
	}
}
...
func TestValidatePGVersion(t *testing.T) {
	tests := []struct {
		...
		{input: "14", want: 14},
		{input: "18", want: 18},
		{input: betaPGMajorString(), want: ext.PostgresBetaMajorVersion},
		{input: "12", wantErr: true},
		{input: fmt.Sprintf("%d", ext.PostgresBetaMajorVersion+1), wantErr: true},
