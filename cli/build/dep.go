...
func InstallExtensionDeps(packages []string, pgVersion string) error {
	if len(packages) == 0 {
		return fmt.Errorf("no packages specified")
	}

	logrus.Info(strings.Repeat("=", 58))
	logrus.Infof("[DEPENDENCE] %s", strings.Join(packages, ", "))
	logrus.Info(strings.Repeat("=", 58))

	switch config.OSType {
	case config.DistroEL:
		return installRpmDeps(packages, pgVersion)
	case config.DistroDEB:
		return installDebDeps(packages, pgVersion)
	default:
...
func installRpmDeps(packages []string, pgVersion string) error {
	...
	// Default PG version if not specified
	if pgVersion == "" {
		pgVersion = "16"
	}
...
// Install DEB build dependencies
func installDebDeps(packages []string, pgVersion string) error {
	...
	for _, pkg := range packages {
		...
		if err != nil {
			logrus.Warnf("Failed to read control file for %s: %v", pkg, err)
			continue
		}
	...
	if len(deps) > 0 {
		...
		logrus.Infof("Installing %d dependencies", len(depList))
		cmd := append([]string{"apt", "install", "-y"}, depList...)
