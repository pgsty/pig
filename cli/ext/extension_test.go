	...
	tests := []struct {
		...
		},
		{
			...
			record: []string{
				...
				"f", "t", "f", "f", "t", "f", "f", "f", "",
				"{17,16,15,14}", "{columnar,pg_cron,pg_ivm,pg_partman}", "", "{timescaledb,timescaledb_toolkit}",
				"0.1.6", "PIGSTY", "pg_timeseries_$v", "{17,16,15,14}", "{hydra_$v,pg_cron_$v,pg_ivm_$v,pg_partman_$v}",
				"0.1.6", "PIGSTY", "postgresql-$v-pg-timeseries", "", "{17,16,15,14}",
