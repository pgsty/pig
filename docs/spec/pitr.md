...
The side-restore directory must exist and be owned by the configured DBSU; unlike managed PGDATA, it does not need to be initialized with `PG_VERSION` before restore. Pig does not create side-restore directories automatically, even though pgBackRest can create some restore paths by itself, because Pig needs a concrete directory for side-restore classification, owner checks, and safe post-restore guidance before it runs the destructive restore. Prepare the directory explicitly, for example:

```bash
install -d -m 700 -o postgres -g postgres /tmp/pg-restore
```

The side-restore directory must be empty enough for pgBackRest restore to write into it; if it contains unrelated files, pgBackRest may require cleanup or explicit restore options. If the requested side directory cannot be canonicalized, Pig treats it as a side restore and lets the normal data-directory precheck report the concrete existence or owner error. If the requested side directory resolves successfully but the managed PGDATA cannot be resolved, the explicit `-D` is still treated as a side restore rather than failing the precheck on the unrelated managed path.
