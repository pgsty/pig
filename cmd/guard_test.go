...
var shorthandPermitTable = map[string]string{
	"patroni -U":             "dbsu",
	"patroni failover -c":    "candidate",
	"patroni failover -f":    "force",
	...
	"patroni pause -w":       "wait",
	"patroni reinit -f":      "force",
	"patroni reinit -w":      "wait",
	"patroni restart -f":     "force",
	"patroni restart -p":     "pending",
	"patroni restart -r":     "role",
	"patroni resume -w":      "wait",
	"patroni switchover -c":  "candidate",
	"patroni switchover -f":  "force",
	"patroni switchover -l":  "leader",
	"patroni switchover -s":  "scheduled",
	"pgbackrest -U":          "dbsu",
	"pgbackrest -c":          "config",
	"pgbackrest -r":          "repo",
	"pgbackrest -s":          "stanza",
	...
	"pgbackrest delete -f":   "force",
	"pgbackrest delete -y":   "yes",
	"pgbackrest expire -y":   "yes",
	"pgbackrest info -O":     "raw-output",
...
func TestT2GateFlagLint(t *testing.T) {
	gates := []struct {
		...
		{[]string{"pgbackrest", "delete"}, "force"}, // B05 pending: flips to yes
		{[]string{"pitr"}, "yes"},
		{[]string{"patroni", "restart"}, "force"}, // B04 pending: flips to yes
		{[]string{"patroni", "reinit"}, "force"},
		{[]string{"patroni", "switchover"}, "force"},
		{[]string{"patroni", "failover"}, "force"},
	}
	for _, g := range gates {
