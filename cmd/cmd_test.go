...
func TestCriticalCommandsExposePlanContract(t *testing.T) {
	cmds := allCommands(rootCmd)
	...
	for _, cmd := range cmds {
		...
		schema := ancs.FromAnnotations(cmd.Annotations)
		if schema == nil || schema.Risk != "critical" {
			continue
		}
		t.Run(cmd.CommandPath(), func(t *testing.T) {
			if schema.Confirm != "required" {
				t.Fatalf("critical command confirm=%q, want required", schema.Confirm)
			}
			if cmd.Flag("plan") == nil && cmd.Annotations["plan"] == "" {
				t.Fatalf("critical command must support --plan or set annotation plan=<reason>")
			}
		})
	}
}
